...
LOGOUTREQUEST;
    ...
    {
        if ($request instanceof DOM) {
            $dom = $request;
        } else {
            $dom = DOM::get()->load($request)->getDOM();
        }
    ...
    public static function getNameIdData($request, $key = null)
    {
        ...
        $encryptedEntries = Utils::query($dom, Constants::SELECTOR_LOGOUT_ENCRYPTED_ID);
        ...
        if ($encryptedEntries->length == 1) {
            ...
            $entries = Utils::query($dom, Constant);
            if ($entries->length == 1) {
    ...
    public static function getIssuer($request)
    {
        ...
        $issuer = null;
        $issuerNodes = Utils::query($dom, Constants::SELECTOR_LOGOUT_ISSUER);
    ...
    public static function getSessionIndexes($request)
    {
        ...
        $sessionIndexNodes = Utils::query($dom, Constants::SELECTOR_LOGOUT_SESSION_INDEX);
