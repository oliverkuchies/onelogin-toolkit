...
class Response
{
     ...
     */
    public function __construct($settings, $response)
    {
        ...
        $this->document = DOM::get()->load($this->response)->getDOM();
        if (!$this->document) {
    ...
    public function isValid($requestId = null)
    {
        if (isset($requestId)) {
            ...
            try {
                ...
                if ($this->encrypted) {
                    $encryptedIDNodes = Utils::query($this->decryptedDocument, Constants::SELECTOR_ASSERTION_SUBJECT_ENCRYPTED_ID);
    ...
    public function getIssuers()
    {
        ...
        $responseIssuer = Utils::query($this->document, Constants::SELECTOR_RESPONSE_ISSUER);
    ...
    protected function decryptAssertion(\DomNode $dom)
    {
        ...
        $decrypted = DOM::get()->load($decryptedXML)->getDOM();
        if ($decrypted === false) {
        ...
        if ($encData->parentNode instanceof DOMDocument) {
            ...
            return DOM::get()->load($container->ownerDocument->saveXML())->getDOM();
        }
    }
